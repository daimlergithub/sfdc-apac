    ...
    for(QC__c qc : Trigger.new) {
		...
		*/
        ...
        if(qc.FMSK__c != Trigger.oldMap.get(qc.Id).FMSK__c) {
            QCHistorys.add(QCHelper.newQCHistory(qc.Id, 'Familiar Central/Smart/ Knowledge', qc.FMSK__c, Trigger.oldMap.get(qc.Id).FMSK__c));
        }
        if(qc.Q6_Comments__c != Trigger.oldMap.get(qc.Id).Q6_Comments__c) {
            QCHistorys.add(QCHelper.newQCHistory(qc.Id, 'Familiar Central/Smart/ Knowledge Comments', qc.Q6_Comments__c, Trigger.oldMap.get(qc.Id).Q6_Comments__c));
        }
